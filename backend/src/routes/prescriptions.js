...
// ============================================================
// PUBLIC: Read prescription image with AI (no auth, no DB save)
// ============================================================
router.post('/read-ai', async (req, res) => {
  try {
    const { image, mimeType, doctorName, hospitalName } = req.body;
    ...
    // Run AI extraction
    const aiResult = await readPrescriptionFromBase64(base64Clean, mime);

    // Format medicines for frontend
    const medicines = (aiResult.medicines || []).map((m, idx) => ({
      id: idx + 1,
    ...
    res.json({
      id: 'rx-' + Date.now(),
      doctorName: aiResult.doctor_name || doctorName || '',
      hospitalName: aiResult.hospital_name || hospitalName || '',
    ...
    console.error('Read-AI prescription error:', error);
    res.status(500).json({ error: 'Failed to read prescription with AI: ' + error.message });
  }
});
