  ...
  login: async (phone, password) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        localStorage.setItem('rxmax_store_id', (data.store && data.store.id) || data.storeId);
        return data;
      }
  ...
  register: async (storeData) => {
    try {
      ...
      if (response.ok) {
        ...
        return { ...data, storeId, patientLink: 'https://rxmax-patient-app.vercel.app/store/' + ((data.store && data.store.slug) || storeId) };
      }
    } catch (error) {
