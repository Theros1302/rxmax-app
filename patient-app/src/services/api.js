...
class ApiService {
  ...
  async uploadPrescription(imageData, doctorName, hospitalName) {
    ...
    try {
      const result = await this.apiCall('POST', '/prescriptions/upload-base64', {
        image: base64Clean,
        mimeType,
        doctorName,
        hospitalName
      ...
      console.log('Server unavailable for prescription upload, trying Gemini directly:', err.message);
    }
